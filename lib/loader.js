...
const editContentPatch = (
  ...
  waitForElm(sel)
    .then((elm) => {
      const html = elm.outerHTML;
      const cnt = callback(elm.outerHTML);
      elm.outerHTML = cnt;
      log(`Edited ${sel}, ${cnt}`);
      const curHTML = elm.outerHTML;
      if (continuous && oldHTML != html)
        editContentPatch(sel, callback, true, curHTML);
    })
...
const patches = [
  ...
  {
    name: "Remove lighter part of sidebar",
    exec: () => {
      editContentPatch(
        "div.nav-alt-bottom-container",
        (content) =>
          content.replace(
            "<div ",
...
window.deleteElementPatch = deleteElementPatch;
window.editContentPatch = editContentPatch;
